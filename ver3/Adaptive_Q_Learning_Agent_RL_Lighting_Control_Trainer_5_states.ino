 ...
 * @brief Embedded Reinforcement Learning Agent Implementation for Adaptive Lighting Control.
 * @project Embedded Real-Time Adaptive System (ERTAS)
 *
 * This module implements a core **Adaptive Q-Learning Agent** on the ESP32-S3 microcontroller
 * to automatically control an LED's brightness (PWM) based on real-time ambient light (Lux)
 * measured by a BH1750 sensor.
 * The agent's goal is to maintain a target Lux level (TARGET_LUX)
 * while minimizing the energy consumed by the LED actuator.
 *
 * ---
 *
 * ## 1. CORE RL MECHANISM: Q-LEARNING
 *
 * The agent learns the optimal policy ($\pi^*$) through interaction with the environment
 * using the Q-Learning algorithm.
 * The policy is stored in the **Q-table** $Q(s, a)$,
 * which estimates the expected maximum discounted future reward for taking action $a$
 * in state $s$.
 *
 * ### Q-Value Update Rule (Implemented in `updateQTable`):
 ...
 *
 * This version implements **self-tuning** to optimize the learning process:
 *
 * ### A. Adaptive Exploration ($\epsilon$-Decay):
 * - **Mechanism:** The global variable `currentEpsilon` starts at `EPSILON_START` (1.0)
 * and decays exponentially until it reaches `EPSILON_END` (0.05).
 * - **Benefit:** Forces **high exploration** initially, then automatically transitions to
 * **high exploitation** for stable control.
 *
 * ### B. Dynamic Learning Rate ($\alpha$-Decay):
 * - **Mechanism:** The learning rate ($\alpha$) is calculated dynamically based on the 
 * visitation count ($N$) of a specific state-action pair, ensuring stability for well-learned
 * states and high adaptability for new situations.
 *
 * ---
 *
 * ## 3. CONTROL LOOP OPTIMIZATION & TIGHTER LUX CONTROL
 *
 * - **5-Bin State Space:** The state space is expanded to 5 bins (**Very Low, Low, Target (250-350 Lux), High, Very High**) to force the agent to learn finer distinctions around the 300 Lux target.
 * - **High Reward Sensitivity:** `REWARD_MAX_DISTANCE` is set to **1.0f** to harshly penalize Lux errors, ensuring the agent quickly seeks the tighter target band.
 * - **Real-time Graphing:** The web dashboard now uses **Chart.js** to display the **Moving Average Reward** across the last 100 episodes, providing a visual confirmation of learning convergence.
 */
 ...
 * The high sensitivity (REWARD_MAX_DISTANCE=1.0f) harshly penalizes Lux outside 
 * the target band.
 */
float calculateReward(float lux) {
    ...
    // Reward is highly sensitive to error
...
/**
 * @brief Initializes the Q-Table, N-Table, Epsilon, and Episode Count.
 */
void initQTable() {
    ...
    episodeCount = 0; // Reset episode count
}

// --------------------------------------------------------
// --- Embedded HTML/CSS/JavaScript Dashboard Client ---
// --------------------------------------------------------
// NOTE: Includes Chart.js for real-time moving average reward plot.
    ...
    <script>
        ...
        async function fetchRLData() {
            try {
                ...
                // 5. Update Log
                document.getElementById('rl-log').textContent = data.log.replace(/\\/g, '');
                // Handle escaped characters from ESP String
            ...
            setTimeout(fetchRLData, 500);
            // Fetch data every 0.5s to match RL episode time
        }
...
/**
 * @brief Serves the real-time data as JSON, including episode count for graphing.
 */
void handleData() {
 ...
 // 🌟 NEW: Episode count for the chart x-axis 🌟
...
void setup() {
 ...
 Serial.println("--- Starting ERTAS RL Agent ---");
    ...
    initQTable(); // Crucial reset for the new 5-state model
...
void loop() {
 ...
 // --- RL Episode Logic (Runs every 500 ms for fast response) ---
 ...
 // 3. LEARN: Perform Q-Table Update (Applies Epsilon and Alpha Decay)
        updateQTable(S_prev, A_prev, R_current, S_new);
 
 // 4. ACTION: Select Next Action A' (Uses Decaying Epsilon)
 ...
 // 6. SHIFT & TRACK: Set up for Next Iteration
        S_prev = S_new;
        A_prev = A_new; 
 ...
 ALPHA_START / (ALPHA_START + N_table[S_prev][A_prev]) : 0.0f;
        ...
        Serial.printf("Episode: %lu\n", episodeCount); // 🌟 Added episode log 🌟
 ...
 Serial.printf("Epsilon: %.3f | Alpha (Last): %.3f\n", currentEpsilon, log_alpha);
        Serial.println("--------------------------------------------------");
        // ------------------------------------
    }
}
